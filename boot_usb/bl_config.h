...
//
//*****************************************************************************
#define VTABLE_START_ADDRESS    0x1800

//*****************************************************************************
//
// The size of a single, erasable page in the flash.  This must be a power
// of 2.  The default value of 1024 bytes represents the page size for the
// internal flash on all Stellaris MCUs and this value should only be
// overridden if configuring a boot loader to access external flash devices
// with a page size different from this.
//
// Depends on: None
// Exclusive of: None
// Requires: None
...
//*****************************************************************************
//
// Enables support for the code to be compatible with the flash patch that is
// preloaded into the flash of certain Stellaris devices.  This support should
// only be enabled for devices that require it.
//
// Note:  Enabling this feature will cause the boot loader to expect to be
// loaded at 0x1000, despite the fact that debuggers will still think that it
// should be loaded at 0x0.  Therefore, LM Flash Programmer must be used to
// program the boot loader into flash when using this feature.
//
// Depends on: None
// Exclusive of: None
// Requires: None
...
//*****************************************************************************
//
// Indicates that the CAN peripheral operates from a fixed divide of the PLL
// output, meaning that the PLL must be enabled.  This is required by some
// older Stellaris devices, but must not be used on newer Stellaris devices.
// Consult the device data sheet to determine if the CAN peripheral operates
// from a fixed 8 MHz clock derived from the PLL (meaning this option must be
// used) or if it operates from the system clock (meaning this option must not
// be used).
//
